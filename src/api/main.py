nde estfrom fastapi import FastAPI, HTTPException
...
from optimized_places_service import optimized_places_service
...
# Middleware de debug pour diagnostiquer les requêtes
@app.middleware("http")
...
@app.get("/")
async def root():
    """Page d'accueil avec interface trilingue"""
    try:
        ...
        return {
            ...
            "dashboard_url": "http://0.0.0.0:8000/dashboard",
            "quick_links": {
                "dashboard": "/dashboard",
                "routes": "/routes/popular",
                "health": "/health"
            },
            "error": f"Erreur chargement interface: {str(e)}"
        }
...
async def chat_with_ai_complete(request: dict):
    ...
    try:
        ...
        if not message or message.strip() == "":
            response = chat_service._get_fallback_response("empty", requested_lang)
            return {
                "response": response,
        ...
        # APPEL IA AVEC CHAT_SERVICE : OpenRouter + Fallback intelligent
        ai_response = await chat_service.get_chat_response(message, requested_lang)
...
async def autocomplete_address(query: str, session_token: str = None, limit: int = 5):
    """Auto-complétion intelligente : Google Places + OpenStreetMap avec cache"""
    try:
        if not query:
            return {"error": "Paramètre 'query' requis"}
        
        # Utiliser le service optimisé avec fallback intelligent
        results = await optimized_places_service.autocomplete_address(query, limit)
        
        if results:
            return {
                "predictions": results,
                "source": results[0].get('source', 'hybrid'),
                "fallback_used": results[0].get('source') == 'nominatim',
                "cache_hit": optimized_places_service.get_stats()['cache_hits'],
                "total_requests": optimized_places_service.get_stats()['total_requests']
            }
        else:
            return {
                "predictions": [],
...
# @app.get("/places/search-bakeries")  # TEMPORAIREMENT DÉSACTIVÉ
...
# @app.get("/places/geocode")  # TEMPORAIREMENT DÉSACTIVÉ
...
# @app.get("/places/details/{place_id}")  # TEMPORAIREMENT DÉSACTIVÉ
# async def get_place_details(place_id: str):
#     """Détails d'un lieu"""
#     try:
#         details = await hybrid_places_service.get_place_details(place_id)
