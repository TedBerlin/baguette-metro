...
# Import des utilitaires de cartographie
try:
    from map_utils import create_route_map, display_map_in_streamlit, generate_sample_bakeries
except ImportError:
    # Fallback si le module n'existe pas
    def create_route_map(*args, **kwargs):
        return None
    def display_map_in_streamlit(*args, **kwargs):
        st.info("🗺️ Carte en cours de développement")
    def generate_sample_bakeries(*args, **kwargs):
        return []

# Import de l'API PRIM RATP
try:
    from ratp_api import (
        calculate_augmented_eta, RATPAPIClient, get_metro_lines_info, 
        format_eta_display, get_metro_lines_info
    )
except ImportError:
    # Fallback si le module n'existe pas
    def calculate_augmented_eta(*args, **kwargs):
        return {"final_eta": 0, "delays": {"average": 0}, "status": {"departure": "normal", "arrival": "normal"}}
    def RATPAPIClient(*args, **kwargs):
        return None
    def get_metro_lines_info(*args, **kwargs):
        return []
    def format_eta_display(*args, **kwargs):
        return "0 min"
...
with tab1:
    ...
    if st.button(
        ...
        with st.spinner("🧮 Calcul du trajet optimal..."):
            ...
            # Calculer l'ETA augmenté avec l'API PRIM RATP
            departure_station = st.session_state.get('departure_name', 'châtelet').lower()
            arrival_station = st.session_state.get('arrival_name', 'république').lower()
            
            augmented_eta = calculate_augmented_eta(
                base_eta=int(base_eta),
            ...
            st.session_state.route_results = {
                ...
                "augmented_eta": round(augmented_eta["final_eta"], 1),
                ...
                "ratp_data": {
                    "delays": augmented_eta["delays"],
                    "status": augmented_eta["status"],
                    "crowding": augmented_eta["crowding"]
                }
            }
...
with tab2:
    if 'route_results' in st.session_state:
        ...
        # ETA augmenté avec données temps réel RATP
        st.markdown("---")
        st.markdown("#### 🚇 ETA Augmenté - Données Temps Réel RATP")
        
        col1, col2, col3 = st.columns(3)
        
        with col1:
            # ETA augmenté
            augmented_eta = st.session_state.route_results.get('augmented_eta', st.session_state.route_results['bakery_eta'])
            delays = st.session_state.route_results.get('ratp_data', {}).get('delays', {'average': 0})
            
            if delays['average'] > 0:
                st.metric(
                    "🚇 ETA Augmenté", 
                    f"{augmented_eta} min", 
                    f"+{delays['average']:.1f} min de retard",
                    delta_color="inverse"
                )
            else:
                st.metric(
        ...
        with col2:
            # Statut des stations
            status = st.session_state.route_results.get('ratp_data', {}).get('status', {})
            departure_status = status.get('departure', 'normal')
            arrival_status = status.get('arrival', 'normal')
            
            if departure_status == 'normal' and arrival_status == 'normal':
                st.metric(
                    "📊 Statut RATP", 
                    "✅ Normal", 
                    "Toutes les lignes"
        ...
        with col3:
            # Niveau d'affluence
            crowding = st.session_state.route_results.get('ratp_data', {}).get('crowding', {})
            avg_crowding = crowding.get('departure', 'low')
            
            crowding_emoji = {"low": "🟢", "medium": "🟡", "high": "🔴"}
            crowding_text = {"low": "Faible", "medium": "Moyenne", "high": "Élevée"}
            
            st.metric(
                "👥 Affluence", 
                f"{crowding_emoji.get(avg_crowding, '🟢')} {crowding_text.get(avg_crowding, 'Faible')}", 
                "Niveau moyen"
            )
        ...
        # Créer la boulangerie recommandée avec géolocalisation
        recommended_bakery = {
            'name': st.session_state.route_results['bakery_name'],
            'address': st.session_state.route_results['bakery_address'],
            'lat': 48.8566 + random.uniform(-0.02, 0.02),  # Coordonnées réalistes autour de Paris
            'lng': 2.3522 + random.uniform(-0.02, 0.02),
            'rating': st.session_state.route_results['bakery_rating'],
            'specialties': ['Baguette tradition', 'Croissants au beurre', 'Pain au chocolat', 'Éclair au café', 'Tarte aux fruits']
        }
        
        # Générer des boulangeries d'exemple supplémentaires
        additional_bakeries = generate_sample_bakeries(
            st.session_state.departure_coords,
            st.session_state.arrival_coords,
            count=1
        )
        
        # Combiner boulangerie recommandée + supplémentaires
        bakeries = [recommended_bakery] + additional_bakeries
        
        # Créer et afficher la carte
        try:
            route_map = create_route_map(
                departure_coords=st.session_state.departure_coords,
                departure_name=st.session_state.get('departure_name', 'Départ'),
                arrival_coords=st.session_state.arrival_coords,
                arrival_name=st.session_state.get('arrival_name', 'Arrivée'),
                bakeries=bakeries,
                recommended_index=0  # La première boulangerie est la recommandée
            )
            
            if route_map:
                display_map_in_streamlit(route_map, height=400)
                st.caption("🗺️ Carte interactive : Cliquez sur les marqueurs pour plus d'informations")
                ...
                with col4:
                    st.markdown("🟠 **Autres boulangeries**")
            else:
                st.info("🗺️ Carte en cours de chargement...")
                
        except Exception as e:
            st.warning(f"🗺️ Erreur de chargement de la carte : {str(e)}")
            st.info("🗺️ Carte en cours de développement")
...
with tab4:
    ...
    # Métriques API PRIM RATP avec cache intelligent
    st.markdown("---")
    st.markdown("##### 🚇 Métriques API PRIM RATP (Cache Intelligent)")
    
    try:
        ratp_client = RATPAPIClient()
        
        # Affichage du statut du cache
        cache_status = ratp_client.get_cache_status()
        st.info(f"🧠 **Stratégie :** {cache_status['strategy']} | 📦 **Cache :** {cache_status['cache_size']} éléments | ⏱️ **Durée :** {cache_status['cache_duration_minutes']} min")
        st.info(f"🔑 **Clé active :** {cache_status['current_key_index'] + 1}/{cache_status['total_api_keys']} | 🚀 **Nouveau jeton 2024 disponible**")
        
        # Boutons d'action
        col1, col2 = st.columns(2)
        
        with col1:
            if st.button("🧹 Vider le cache API PRIM"):
                ratp_client.clear_cache()
                st.success("Cache vidé ! Rechargez la page pour voir les changements.")
                st.rerun()
        
        with col2:
            if st.button("🔑 Tester nouveau jeton 2024"):
                new_token_result = ratp_client.test_new_token_2024()
                if new_token_result['status'] == 'connected':
                    st.success("🎉 Nouveau jeton 2024 opérationnel !")
                else:
                    st.warning(f"⚠️ {new_token_result['message']}")
                st.rerun()
        
        # Test de connectivité PRIM
        connectivity = ratp_client.test_api_connectivity()
        
        col1, col2, col3 = st.columns(3)
        
        with col1:
            if connectivity['status'] == 'connected':
                st.metric("🔗 Connectivité", "✅ Connecté", f"{connectivity['response_time']:.3f}s")
            elif connectivity['status'] == 'rate_limited':
                st.metric("🔗 Connectivité", "⚠️ Rate Limit", "Cache intelligent activé")
            else:
                st.metric("🔗 Connectivité", "❌ Erreur", connectivity['message'])
        
        with col2:
            # Test des isochrones
            isochrones_test = ratp_client.get_isochrones(48.8566, 2.3522, 1800)
            if isochrones_test['status'] == 'success':
                st.metric("🗺️ Isochrones", "✅ Opérationnel", "Données réelles")
            else:
                st.metric("🗺️ Isochrones", "⚠️ Fallback", "Cache intelligent")
        
        with col3:
            # Test des lignes de métro
            lines_test = ratp_client.get_metro_lines()
            if lines_test['status'] == 'success':
                st.metric("🚇 Lignes métro", "✅ Opérationnel", "Données réelles")
            else:
                st.metric("🚇 Lignes métro", "⚠️ Fallback", "Cache intelligent")
        
        # Détails des données PRIM
        if connectivity['status'] == 'connected':
            st.success("🚇 API PRIM RATP opérationnelle avec données temps réel !")
            
            # Affichage des isochrones récentes
            st.markdown("#### 🗺️ Dernières isochrones calculées")
            recent_isochrones = isochrones_test.get('data', {})
            if 'features' in recent_isochrones:
                st.json(recent_isochrones)
            else:
                st.info("Aucune isochrone disponible pour le moment")
        elif connectivity['status'] == 'rate_limited':
            st.warning("⚠️ API PRIM en rate limit - Utilisation du cache intelligent pour optimiser les performances")
        
    except Exception as e:
        st.error(f"Erreur lors du test de l'API PRIM : {str(e)}")
        st.info("L'API PRIM sera testée lors du prochain calcul de trajet")
    
    # Test de l'itinéraire détaillé
    st.markdown("---")
    st.markdown("#### 🚇 Test Itinéraire Détaillé")
    
    try:
        ratp_client = RATPAPIClient()
        journey_test = ratp_client.get_journey_details(48.8584, 2.3470, 48.8674, 2.3636)  # Châtelet → République
        
        if journey_test['status'] == 'success':
            st.success("✅ Itinéraire détaillé disponible via API PRIM !")
            
            # Afficher les détails de l'itinéraire
            journey_data = journey_test.get('data', {})
            if 'journeys' in journey_data:
                st.markdown("**📋 Détails de l'itinéraire Châtelet → République :**")
                
                for i, journey in enumerate(journey_data['journeys']):
                    st.markdown(f"**Option {i+1}** - Durée: {journey.get('duration', 0)//60} min")
                    
                    for j, section in enumerate(journey.get('sections', [])):
                        if section.get('type') == 'public_transport':
                            st.markdown(f"🚇 **{section.get('route', {}).get('name', 'Métro')}**")
                            st.markdown(f"De: {section.get('from', {}).get('name', 'N/A')} → À: {section.get('to', {}).get('name', 'N/A')}")
                            
                            # Afficher les stations intermédiaires
                            if 'stop_points' in section:
                                st.markdown("**Stations :**")
                                stations_text = " → ".join([sp.get('name', 'N/A') for sp in section['stop_points']])
                                st.caption(stations_text)
                        else:
                            st.markdown(f"🚶 **{section.get('mode', 'Marche')}** - {section.get('duration', 0)//60} min")
            else:
                st.info("Aucun itinéraire détaillé disponible pour le moment")
        else:
            st.warning("⚠️ Utilisation des données simulées pour l'itinéraire")
            
            # Afficher l'itinéraire simulé
            mock_journey = journey_test.get('data', {})
            if 'journeys' in mock_journey:
                st.markdown("**📋 Itinéraire simulé Châtelet → République :**")
                
                for i, journey in enumerate(mock_journey['journeys']):
                    st.markdown(f"**Option {i+1}** - Durée: {journey.get('duration', 0)//60} min")
        ...
        st.error(f"Erreur lors du test de l'itinéraire : {str(e)}")
        st.info("L'itinéraire détaillé sera testé lors du prochain calcul de trajet")
