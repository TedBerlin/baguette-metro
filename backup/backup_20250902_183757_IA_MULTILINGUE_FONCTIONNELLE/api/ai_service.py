...
from config import (
    ...
    MISTRAL_API_URL, OPENROUTER_API_URL,
    API_TIMEOUT, MAX_TOKENS, TEMPERATURE, TOP_P,
    ENABLE_MISTRAL_DIRECT, ENABLE_OPENROUTER_FALLBACK, ENABLE_LOCAL_FALLBACK
)
...
async def call_openrouter_api(message: str, language: str) -> str:
    """Appelle l'API OpenRouter (priorité 2)"""
...
async def call_ai_with_fallback(message: str, language: str) -> tuple[str, str]:
    ...
    try:
        ...
        if language == "fr":
            # FRANÇAIS : Fallback local direct (Mistral a un biais anglais)
            print(f"🔵 FRANÇAIS DÉTECTÉ - Utilisation du fallback local")
        ...
        # 2. TENTATIVE OPENROUTER (fallback général)
